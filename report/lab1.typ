...
=== Интерфейс
- `PerfectHash(keys):` Конструктор. Выполняет предварительную фильтрацию дубликатов, распределение по корзинам и итеративный подбор параметров $a, b$ для каждого уровня до тех пор, пока не будет достигнуто отсутствие коллизий.
- `get(key):` Поиск. Возвращает std::optional<int64_t>. Выполняет ровно два вычисления хеша и одну проверку ключа в ячейке.
